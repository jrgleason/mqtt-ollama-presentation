...
**Previous Behavior:** audioPlayer.play() was fire-and-forget (no cancellation)

**New Behavior:** Playback is tracked and cancellable

#### Scenario: Orchestrator interruption method

- **GIVEN** VoiceInteractionOrchestrator has active playback
- **WHEN** interrupt() method is called
- **THEN** active playback SHALL be cancelled
- **AND** "🛑 Interrupting TTS playback" SHALL be logged

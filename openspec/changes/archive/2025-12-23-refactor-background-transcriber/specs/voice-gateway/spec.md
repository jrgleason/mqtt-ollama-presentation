# Voice Gateway Capability Specification

## Overview

The Voice Gateway capability provides voice-controlled interaction with home automation devices and AI assistants. It orchestrates the complete pipeline from audio capture to spoken AI responses, integrating speech recognition, intent classification, AI inference, and text-to-speech.
...
- **BackgroundTranscriber** class is REMOVED in this change
- Code importing `BackgroundTranscriber` MUST be updated to use `VoiceInteractionOrchestrator`
- Method signature changes:
  - OLD: `BackgroundTranscriber.backgroundTranscribe(audioSamples)`
  - NEW: `VoiceInteractionOrchestrator.processVoiceInteraction(audioSamples)`

### Configuration Requirements
- No configuration changes required
- All services use existing config structure (whisper, ollama, anthropic, tts, mqtt)

### Testing Impact
- New unit tests required for TranscriptionService, IntentClassifier, AIRouter
- Integration tests required for VoiceInteractionOrchestrator
- End-to-end hardware testing required before deleting BackgroundTranscriber

### Deployment Strategy
- Create all new services first (additive changes)
- Test independently before integration
- Use feature flag to toggle between old and new implementations
- Verify performance benchmarks match or exceed old implementation
- Delete BackgroundTranscriber only after successful hardware testing

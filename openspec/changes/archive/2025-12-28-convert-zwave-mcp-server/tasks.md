...
- [x] **Task 1.10**: Fix voice gateway after simplify-tool-management refactoring
  - File: `apps/voice-gateway-oww/src/services/ToolExecutor.js`
  - Issue: ToolExecutor was calling `tool.invoke({ input: args })` (wrapped)
  - Root cause: simplify-tool-management refactoring reintroduced the bug
  - Fix: Changed to `tool.invoke(args)` (direct, no wrapper)
  - **Validation**: Voice gateway sensor queries work again
  - **Status**: ✅ FIXED - Voice gateway working with correct tool invocation (line 102)
...
- [x] **Task 3.5**: Test frontend MCP tool discovery
  - Open oracle in browser
  - Check browser console: No errors
  - Verify: Tools are listed in UI (3 tools: list_zwave_devices, control_zwave_device, get_device_sensor_data)
  - **Pass criteria**: UI displays all MCP tools correctly
  - **Status**: ✅ PASS - Model selector shows qwen3:0.6b/qwen2.5:0.5b, MCP tools discovered successfully

- [x] **Task 3.6**: Test frontend device list
  - Ask via chat: "What devices are available?"
  ...
  - **Status**: ✅ PASS - Returns "Node 1" and "Temp Sensor 1" (with qwen3:0.6b)
  - **Note**: qwen2.5:0.5b failed (too small for tool calling), qwen3:0.6b succeeded
...
- [x] **Task 3.9**: Fix Oracle tool invocation bug
  - File: `apps/oracle/src/app/api/chat/route.js`
  - Issue: Tool results returned as `[text_content, artifacts_array]` but LangChain expects string
  - Fix: Added normalization to extract text from MCP array format
  - **Validation**: Tools execute without "Non string tool message content" error
  - **Status**: ✅ FIXED - Added tool result normalization (lines 163-175)

- [x] **Task 3.10**: Fix Oracle system prompt
  - File: `apps/oracle/src/app/api/chat/route.js`
  - Issue: System prompt referenced wrong tool names (list_devices vs list_zwave_devices)
  - Fix: Updated to use correct MCP tool names
  - **Validation**: AI calls tools instead of asking for parameters
  - **Status**: ✅ FIXED - Updated system prompt with correct tool names (lines 119-126)

- [x] **Task 3.11**: Test Oracle with sensor query
  - Ask via chat: "What is the temperature on temp sensor 1?"
  - Verify: Backend calls get_device_sensor_data tool
  - Verify: Temperature reading returned
  - **Pass criteria**: Sensor query works from browser
  - **Status**: ✅ PASS - Returns temperature successfully (tested with qwen3:0.6b)
...
1. ✅ **Voice gateway uses LangChain MCP adapter** - Phase 1 complete, all tools working
2. ✅ **Oracle backend MCP integration** - Chat route uses MultiServerMCPClient for tool discovery
3. ✅ **MCP tools work correctly** - Device list and sensor queries verified on both voice gateway and Oracle
4. ✅ **No custom `MCPZWaveClient` code remains** - All references removed
5. ✅ **Bug fixes applied**:
   - Fixed tool invocation wrapper issue (voice gateway)
   - Fixed tool result normalization (Oracle)
   - Fixed system prompt tool names (Oracle)

### Optional/Deferred Items

- ⏸️ **Device control testing** - Blocked (no controllable devices, only temp sensor)
- ⏸️ **Phase 4 enhancements** - Optional (SSE transport, caching, systemd)
- ⏸️ **Phase 6 integration tests** - Deferred (requires full integration test suite)
- ⏸️ **Phase 7 cleanup** - Deferred (documentation, linting)

### Known Limitations

1. **Model Selection**: qwen2.5:0.5b too small for reliable tool calling, qwen3:0.6b+ recommended
2. **Markdown Rendering**: Oracle chat UI shows raw markdown syntax (addressed in separate proposal: fix-oracle-chat-ui)
3. **Device Control**: Cannot fully test without controllable Z-Wave devices (switch/dimmer)

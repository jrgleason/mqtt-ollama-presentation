...
The voice gateway and Oracle applications have the standard LangChain MCP adapters installed but are using custom client wrappers instead. This creates maintenance burden, duplicates functionality, and prevents both applications from benefiting from auto-discovered tools and standard MCP integration patterns. Converting to use the installed `@langchain/mcp-adapters` library eliminates custom code, enables multi-consumer support, and follows industry best practices.

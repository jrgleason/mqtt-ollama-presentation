...
### Requirement: Manual Tool Format Conversion

~~Voice-gateway SHALL convert LangChain tools to Qwen-compatible format before passing to Ollama API.~~

**Rationale:** This requirement is obsolete because LangChain's `.bindTools()` handles all tool format conversion internally, eliminating the need for manual conversion and associated performance overhead.

#### Scenario: Convert LangChain tool to Qwen format (REMOVED)
...
**Rationale:** LangChain's ChatOllama adapter performs this conversion automatically and efficiently.

#### Scenario: Convert multiple tools (REMOVED)
...
**Rationale:** LangChain's `.bindTools()` handles batch conversion optimally.

#### Scenario: Log tool conversion (REMOVED)
...
**Rationale:** LangChain's internal logging provides sufficient visibility into tool binding.

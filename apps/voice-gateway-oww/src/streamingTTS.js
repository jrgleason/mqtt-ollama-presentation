...
const elevenLabsTTS = new ElevenLabsTTS(config, logger);

// Wrapper function to match expected signature
const elevenSynthesize = (text, options) => elevenLabsTTS.synthesizeSpeech(text, options);
...
export async function streamSpeak(text, options = {}) {
    ...
    function maybeFlush(reason) {
        ...
        if (!haveWaited && totalFlushChars < minChunkChars && buffer.length < maxBufferChars) {
            logger.debug('⌛ streamSpeak: initial delay not elapsed', {
                reason,
                waitedMs: firstTokenAt ? (now - firstTokenAt) : 0
            });
            return;
        }
        ...
        if (totalFlushChars < minChunkChars && buffer.length < maxBufferChars) {
            logger.debug('⌛ streamSpeak: buffering (minChunkChars)', {
                reason,
                totalFlushChars,

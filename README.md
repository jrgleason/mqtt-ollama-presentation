...
3. Follow the service setup checklist in [Getting Started][get-started] to configure MQTT, zwave-js-ui, Oracle, and
...
- The Docker Compose file targets local development. Disable the `voice` profile if you do not need microphone/speaker
  passthrough.
- Ollama must be reachable at `http://host.docker.internal:11434` from containers; adjust `OLLAMA_BASE_URL` if you host
  it elsewhere.
- Use `npm test` within service directories to run Jest suites. Feature specs live alongside code in `__tests__`
  folders.

...
This page gives the fastest path to a functioning demo. Use it as a checklist, then dive into the linked runbooks when you need deeper detail.
...
- Follow the hardware preparation guide for the Z-Pi 7 HAT, audio devices, and cooling. See [Raspberry Pi 5 Setup][pi-setup].
- Install Node.js 20 (via NVM), Docker, Docker Compose, and Ollama. If Ollama runs on another machine, note its URL for later.
...
2. If you plan to use voice support, repeat the env setup for `apps/voice-gateway-oww` (`.env.example` is inside that folder) and run `npm install --prefix apps/voice-gateway-oww`.
3. For Pi deployments, provision Mosquitto and zwave-js-ui services using the steps in [MQTT Setup][mqtt-setup] and [zwave-js-ui Deploy][zwave-deploy].
...
- Explore voice control, wake-word tuning, and TTS pipelines via the [Voice Gateway Overview][voice-doc] and related audio notes.

...
The Jest config had `roots: ['<rootDir>/src', '<rootDir>/tests']`, but the `tests/` directory was empty. Git doesn't
track empty directories, so in CI the directory didn't exist, causing Jest validation to fail.
...
However, remember that these are temporary fixes. The tasks.md file now tracks 17 action items for writing real tests
before the demo to achieve 60%+ coverage.
